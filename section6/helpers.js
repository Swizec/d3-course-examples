...
var prepare = {
    ...
    ufos_by_season: function (ufos, cluster_assignment) {
        ...
        ufos = ufos.map(function (ufo, i) {
            ufo.cluster = cluster_assignment[i];
            return ufo;
        });
    ...
    precalc_animation: function (ufos_by_season, geo_projection, centroids) {
        var seasons = d3.scale.ordinal()
            ...
            start_year = 1945,
            base_state = {centroids: 
                          prepare.centroid_base_state(
                              centroids.centroids,
                              centroids.clustered,
...
var make_keyframe = function make_keyframe (ufos_by_season, geo_projection,
    ...
    var currently_drawn =
        d3.sum(sum.centroids.map(function (d) { return d.count; }))
            +d3.sum(_.values(cluster_ids).map(
    ...
    var R = d3.scale.linear()
            .domain([0, 
                     d3.max(sum.centroids.map(function (d) { return d.R; }))])
            .range([0, 20]);
